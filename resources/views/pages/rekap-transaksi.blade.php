    ...
    <div class="row justify-content-end">
        <div class="col-12 position-relative">
            ...
            <div class="dashboard p-3 shadow-sm mb-5">
                <div class="bg-light rounded-circle d-flex justify-content-center align-items-center border"
                    id="profile-foto" style="width: 125px; height: 125px;cursor:pointer;">
                    @if (Auth::user()->avatar)
                        <img src ="/assets/img/profile/{{ Auth()->user()->avatar }}" class="rounded-circle" width="125x"
                            height="125px">
                    @else
                ...
                <div class="mb-4 mt-5 p-1  @if (Route::currentRouteName() == 'dashboard.index') active @endif "><a
                        href="{{ route('dashboard.index') }}" class="text-secondary"><svg
                            data-src="/assets/img/dashboard-profile/dashboard.svg" class="me-2" width="20"
                            height="20" fill="#828282"></svg>Dashboard</a></div>
                <div class="mb-4 @if (Route::currentRouteName() == 'dashboard.profile.index') active @endif p-1"><a
                        href="{{ route('dashboard.profile.index') }}" class="text-secondary"><svg
                            data-src="/assets/img/dashboard-profile/user.svg" class="me-2" width="20"
                            height="20"></svg>Profile</a></div>
                ...
                <div class="mb-4 @if (Route::currentRouteName() == 'dashboard.show.rekap.transaksi') active @endif p-1"><a
                        href="{{ route('dashboard.show.rekap.transaksi') }}" class="text-secondary"><img
                            src="/assets/img/dashboard-profile/transaction.svg" alt="" width="20" height="20"
                            fill="#828282" class="me-2">Rekap Transaksi</a></div>
        ...
        <div class="col-md-9 p-4" style="min-height: 90vh;margin-top:-80px">
                    ...
                    <div class="card position-relative shadow-sm mb-3">
                        ...
                        <div class="card-top d-flex row align-items-center">
                            ...
                            <div class="py-3 px-4 col-4 text-center">
                                <div class="p-0">
                                    <p
                                        class="status @if ($item->status == 'PAID') selesai @elseif($item->status == 'PENDING' || $item->status == 'UNPAID(BUTUH BAYAR)') menunggu @else gagal @endif ">
                                        @if ($item->status == 'PAID')
                                            Selesai
                                        @elseif($item->status == 'PENDING')
                                            Menunggu Pembayaran
                                        @elseif($item->status == 'UNPAID(BUTUH BAYAR)')
                                            Langkapi Metode Pembayaran
                        ...
                        <div class="card-middle d-flex px-4 py-3 gap-3 ">
                            ...
                            <div class="texts">
                                <p class="m-0">
                                    [{{ $item->paket_profesional_conselings->professional_conseling->jenis_layanan }}] Paket
                                    - {{ $item->paket_profesional_conselings->nama_paket }} -
                                    {{ $item->paket_profesional_conselings->professional_conseling->namaPengalaman }}</p>
                                <span>{{ $item->paket_profesional_conselings->jumlah_sesi }} sesi</span>
                            </div>
                        </div>

                        <div
                            class="card-bottom d-flex flex-column flex-lg-row align-items-center justify-content-between px-4 py-3">
                            <div class="card-bottom-content">
                                <span>Jadwal</span>
                                <p>{{ \Carbon\Carbon::parse($item->detail_pembayarans->tgl_konsultasi)->locale('id_ID')->format('d F Y') }}
                                    / {{ $item->detail_pembayarans->jam_konsultasi }} WIB</p>
                            </div>
                            <div class="card-bottom-content">
                                <span>Konselor</span>
                            ...
                            <div class="card-bottom-content">
                                ...
                                @elseif($item->status == 'PENDING' || $item->status == 'UNPAID(BUTUH BAYAR)')
                                    @if ($item->status == 'UNPAID(BUTUH BAYAR)')
                                        <a href="{{ route('professional.konseling.checkout', $item->ref_transaction_layanan) }}"
                                            class="btn btn-join me-2 py-2 px-3 rounded-3"
                                            style="background-color: #2139F9;">Lanjutkan</a>
                                        <a href="#" class="btn btn-join me-2 py-2 px-3 rounded-3"
                                            style="background-color: #D60F27;">Batalkan</a>
                                        <a href="https://wa.me/6282142625552"
                                            class="btn btn-admin bg-secondary py-2 px-3 rounded-3">Hubungi admin</a>
                                    @elseif($item->status == 'PENDING')
                                        <a href="#" class="btn btn-join me-2 py-2 px-3 rounded-3"
                                            style="background-color: #F9D770;">Pending</a>
                                        <a href="https://wa.me/6282142625552"
                                            class="btn btn-admin bg-secondary py-2 px-3 rounded-3">Hubungi admin</a>
                                    @endif
                                @elseif($item->status == 'EXPIRED')
                ...
                @elseif($item->paket_non_professionals != null)
                    <div class="card position-relative shadow-sm mb-3">
                        ...
                        <div class="card-middle d-flex px-4 py-3 gap-3 ">
                            ...
                            <div class="texts">
                                <p class="m-0">
                                    [{{ $item->paket_non_professionals->layanan_non_professionals->jenis_layanan }}]
                                    {{ $item->paket_non_professionals->nama_paket }} -
                                    {{ $item->paket_non_professionals->layanan_non_professionals->nama_layanan }} </p>
            ...
            @empty
                <center>
                    <div class="m-5 p-5">
                        <img src="/assets/img/dashboard-profile/no_transaction.png" class="mb-5 img-fluid" width="70%"
